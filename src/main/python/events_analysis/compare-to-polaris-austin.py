...
legs.loc[(legs.prevActivityType == "PrimaryWork") & (legs.nextActivityType == "home"), "tourType"] = "HBW"
...
actBinCounts = np.round(actBinCounts / 0.2)
...
CountyToCountyFlow =  CountyToCountyFlow.loc[CountyToCountyFlow > 10].unstack().fillna(0) / 0.2
...
CountyToCountyFlow.unstack().sort_index().to_csv(pwd + outfolder + "CountyToCountyFlowUnstacked.csv")

#%%

events = pd.read_csv(urbansim+ '15.events.csv.gz')
...
#%%
Energy = pd.Series()
Energy['Fuel_Auto'] = gb_mode_fuel.loc[['car','car_hov2','car_hov3'],'gallons'].unstack()['Gasoline'].sum() / 0.2 / 1000000
Energy['Fuel_TNC'] = gb_mode_fuel.loc['car_RH','gallons']['Gasoline'] / 0.2 / 1000000
Energy['Fuel_Transit'] = gb_mode.loc['bus','vehicleMiles'] / 1000000 / 3.71
Energy['Electricity_Auto'] = gb_mode_fuel.loc[['car','car_hov2','car_hov3'],'MWH'].unstack()['Electricity'].sum() / 0.2
Energy['Electricity_TNC'] = gb_mode_fuel.loc['car_RH','MWH']['Electricity'] / 0.2 
Energy['Total_Auto'] = gb_mode_fuel.loc[['car','car_hov2','car_hov3'],'MWH'].unstack()['Gasoline'].sum() / 0.2 / 1000 + gb_mode_fuel.loc['car','MWH']['Electricity'] / 0.2 / 1000
Energy['Total_TNC'] = gb_mode_fuel.loc['car_RH','MWH']['Gasoline'] / 0.2 / 1000 + gb_mode_fuel.loc['car_RH','MWH']['Electricity'] / 0.2 / 1000
...
#%%
Summary = pd.Series()
Summary['TotalTrips_Auto'] = np.sum(legs.legMode.isin(['car','drive_transit','ride_hail','ride_hail_pooled','ride_hail_transit','car_hov2','car_hov3'])) / 0.2
Summary['PMT'] = s['passengerMiles'] / 0.2 / 1e6
Summary['PHT'] = s['passengerHours'] / 0.2 / 1e6
Summary['VMT'] = gb_mode_simple.loc[['car','car_hov2','car_hov3'],'vehicleMiles'].sum() / 0.2 / 1e6
Summary['VHT'] = gb_mode_simple.loc[['car','car_hov2','car_hov3'],'vehicleHours'].sum() / 0.2 / 1e6
Summary['AverageVehicleSpeed'] = gb_mode_simple.loc[['car','car_hov2','car_hov3'],'vehicleMiles'].sum() / gb_mode_simple.loc[['car','car_hov2','car_hov3'],'vehicleHours'].sum()
...
Summary['PerCapitaPMT'] = s['passengerMiles'] / 0.2 / per.shape[0]
Summary['PerCapitaPHT'] = s['passengerHours'] / 0.2 / per.shape[0]
Summary['PerCapitaVMT'] = (gb_mode_simple.loc['car_hov2','vehicleMiles'] / 0.2 + gb_mode_simple.loc['car_hov3','vehicleMiles'] / 0.2 + gb_mode_simple.loc['car','vehicleMiles'] / 0.2 + gb_mode_simple.loc['bus','vehicleMiles'] + gb_mode_simple.loc['tram','vehicleMiles']) / per.shape[0]
Summary['PerCapitaVHT'] = (gb_mode_simple.loc['car_hov3','vehicleHours'] / 0.2 + gb_mode_simple.loc['car_hov2','vehicleHours'] / 0.2 +gb_mode_simple.loc['car','vehicleHours'] / 0.2 + gb_mode_simple.loc['bus','vehicleHours'] + gb_mode_simple.loc['tram','vehicleMiles']) / per.shape[0]
